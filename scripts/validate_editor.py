...
for scale in [1,1.5]:
    ...
    rules['rules']=[dict(base,id='shared',name='共享规则',group='网络连接',terms=['timeout'],source_file_ids=['fixture-syslog','fixture-kernel'],target='both'),dict(base,id='second',name='第二条规则',group='另一报告分组',terms=['error'],source_file_ids=['fixture-syslog'],target='keywords')]
    rules['log_files'] += [dict(id='fixture-syslog',name='测试系统日志',path='syslog',mode='prefix',container='diagnostic_archive'),dict(id='fixture-kernel',name='测试内核日志',path='kernel.log',mode='exact',container='diagnostic_archive')]
    rules['layout']['file_order']=['fixture-syslog','fixture-kernel']
